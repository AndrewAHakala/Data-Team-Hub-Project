...
summary as (
    select
        case scenario_type
            when 1 then '1. Models with undocumented columns'
            when 2 then '2. Models missing required columns'
            when 3 then '3. Models with incorrect documentation'
        end as issue_type,
        count(*) as model_count,
        listagg(model_name, ', ') within group (order by model_name) as affected_models
    from all_issues
    group by scenario_type
)

-- Show summary followed by details
/*select
    '=== SUMMARY ===' as separator,
    issue_type as "Issue Type",
    model_count as "Count",
    affected_models as "Affected Models"
from summary
union all
select
    '=== DETAILS ===' as separator,
    model_name as "Issue Type",
    null as "Count",
